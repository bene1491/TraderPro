...
import { Upload, X, Sparkles, Save, Trash2, ChevronDown, ChevronUp, AlertTriangle, LogIn } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-md rounded-3xl border p-6 space-y-4 ${card}`}>
        ...
        <p className={`text-sm leading-relaxed ${sub}`}>
          ...
          <strong className={text}> Google Gemini API</strong> übermittelt. Die Bilder werden
        ...
        <div className={`rounded-2xl p-3 text-xs space-y-1 ${dark ? 'bg-yellow-500/10 text-yellow-300' : 'bg-yellow-50 text-yellow-700'}`}>
          ...
          <p>Stelle sicher, dass auf deinen Screenshots <strong>kein Name, keine Kontonummer und keine Depot-ID</strong> sichtbar sind. Nur Positionen und Werte sollten zu sehen sein.</p>
        </div>

        <p className={`text-xs ${sub} opacity-70`}>
          Diese Einwilligung kann jederzeit unter <strong>Konto → Einstellungen</strong> widerrufen werden.
        </p>
        ...
        <div className="flex gap-3 pt-1">
          <button
            ...
            className={`flex-1 py-3 rounded-2xl border text-sm font-medium transition-colors
              ${dark ? 'border-tp-border text-tp-sub hover:text-tp-text' : 'border-tp-border-l text-tp-sub-l hover:text-tp-text-l'}`}
          >
            Ablehnen
          </button>
          <button
            onClick={onAccept}
            className="flex-1 py-3 rounded-2xl bg-tp-blue text-white text-sm font-semibold hover:opacity-90 transition-opacity"
          >
            Einwilligen & starten
          </button>
        </div>
    ...
    <div className={`rounded-2xl border overflow-hidden ${card}`}>
      ...
      >
        <div>
          ...
          {analysis.investment_style && (
            <div className={`text-xs mt-0.5 truncate max-w-[220px] ${sub}`}>
              Stil: {analysis.investment_style}
            </div>
          )}
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-xl text-tp-red hover:bg-tp-red-bg transition-colors"
          >
            <Trash2 size={15} />
          </button>
      ...
      {expanded && (
        <div className={`px-4 pb-4 text-sm leading-relaxed whitespace-pre-wrap border-t ${sub} ${dark ? 'border-tp-border' : 'border-tp-border-l'}`}>
          <div className="pt-3">{analysis.analysis_result}</div>
        </div>
      )}
    </div>
  )
...
export default function Portfolio() {
  ...
  const { analyses, loading: loadingAnalyses, save, remove } = usePortfolioAnalyses()
  ...
  // Show consent modal on first visit if not yet decided
  useEffect(() => {
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
          ...
          <p className={`text-sm max-w-xs ${sub}`}>
            Du hast die Einwilligung zur Datenübermittlung abgelehnt. Um dieses Feature zu nutzen,
            aktiviere es unter <strong>Konto → Portfolio-Analyse</strong>.
          </p>
          <button
  ...
  return (
    <>
      {showConsent && (
        <ConsentModal onAccept={handleAccept} onDecline={handleDecline} dark={dark} />
      )}
      ...
      <div className="space-y-6">
        ...
        <div className={`rounded-3xl border p-5 space-y-4 ${card}`}>
          ...
          <div className={`rounded-2xl p-3 text-xs flex gap-2 ${dark ? 'bg-yellow-500/10 text-yellow-300' : 'bg-yellow-50 text-yellow-700'}`}>
            ...
            <span>Kein Name, keine Kontonummer und keine Depot-ID sichtbar lassen — nur Positionen und Werte.</span>
          </div>
          ...
          {images.length < 5 && (
            ...
            >
              ...
              <span className="text-sm font-medium">
                {images.length === 0 ? 'Screenshots auswählen' : `Weitere hinzufügen (${images.length}/5)`}
              </span>
            </button>
          )}
          <input
        ...
        <div className={`rounded-3xl border p-5 space-y-3 ${card}`}>
          ...
          <textarea
            ...
            placeholder="z.B. Ich bin langfristig orientiert, tech-fokussiert und möchte breit diversifizieren..."
            rows={3}
            className={`w-full px-4 py-3 rounded-2xl border outline-none transition-colors text-sm resize-none ${input}`}
          />
        </div>
          ...
          <div className={`rounded-3xl border p-5 space-y-4 ${card}`}>
            <div className="flex items-center justify-between">
              ...
              {!saved && (
                <button
                  onClick={handleSave}
                  className="flex items-center gap-1.5 text-xs text-tp-blue font-medium hover:opacity-80 transition-opacity"
                >
                  <Save size={14} />
                  Speichern
                </button>
              )}
              {saved && <span className="text-xs text-tp-green font-medium">Gespeichert ✓</span>}
            </div>
            <div className={`text-sm leading-relaxed whitespace-pre-wrap ${sub}`}>
              {result}
            </div>
          </div>
        )}
        ...
        {user && analyses.length > 0 && (
          <div className="space-y-3">
            ...
            {analyses.map(a => (
              <AnalysisCard key={a.id} analysis={a} onDelete={handleDelete} dark={dark} />
            ))}
          </div>
        )}
        ...
        {!user && (
          <button
            ...
            className={`w-full py-3 rounded-2xl border flex items-center justify-center gap-2 text-sm ${sub}
              ${dark ? 'border-tp-border' : 'border-tp-border-l'}`}
          >
            <LogIn size={16} />
            Anmelden um Analysen zu speichern
          </button>
        )}
      </div>
    </>
